...
def fit_global_fixed_tc(df: pd.DataFrame, finf_tail_df: pd.DataFrame, fixed_tc: float):
    ...
    return {
        ...
        "message": res.message,
        "params": params_full,
        "params_reduced": np.asarray(res.x, dtype=np.float64),
...
def save_global_fit_fixed_tc(result: dict, beta: float, fixed_tc: float, outdir: Path):
...
def bootstrap_fixed_tc(df: pd.DataFrame, fit_result: dict, fixed_tc: float, nboot: int, n_jobs: int, seed0: int):
    if nboot <= 0:
        return {
...
def fit_single_vw(tag: str, vw: float, compare_rows: list[dict]):
    ...
    outdir = ROOT / f"results_collapse_fixed_tc_{tag}_H15H20_tosc"
    ...
    beta_payload = cf.find_best_beta(lattice_df, 120)
    cf.save_json(outdir / "best_beta.json", beta_payload)
    cf.plot_beta_scan(beta_payload, outdir, DPI)
    collapsed = cf.compute_x(lattice_df, beta_payload["beta"])
    ...
    global_payload = save_global_fit_fixed_tc(fit_result, beta_payload["beta"], FIXED_TC, outdir)
    bootstrap_payload = bootstrap_fixed_tc(collapsed, fit_result, FIXED_TC, BOOTSTRAP_N, BOOTSTRAP_JOBS, BOOTSTRAP_SEED + int(vw * 1000))
...
def make_compare_outputs(compare_rows: list[dict]):
    outdir = ROOT / "results_collapse_fixed_tc_vw_compare_tosc"
    outdir.mkdir(parents=True, exist_ok=True)
    df = pd.DataFrame(compare_rows).sort_values("vw").reset_index(drop=True)
    df.to_csv(outdir / "vw_collapse_fixed_tc_summary.csv", index=False)
    cf.save_json(outdir / "vw_collapse_fixed_tc_summary.json", df.to_dict(orient="records"))
    ...
    for ax, row in zip(axes.flat, df.itertuples(index=False)):
        ...
        ax.imshow(img)
        ax.axis("off")
        ax.set_title(rf"$v_w={row.vw:.1f}$, $\beta={row.beta:.3f}$, $t_c=1.5$, $r={row.r:.3f}$" + "\n" + rf"rel-RMSE={row.rel_rmse:.4e}")
    fig.tight_layout()
    fig.savefig(outdir / "collapse_overlay_fixed_tc_by_vw.png", dpi=DPI)
    plt.close(fig)
...
def main():
    ...
    summary = {
        ...
        "fixed_t_c": FIXED_TC,
        "t_osc": cf.T_OSC,
        "normalization": "xi = (x/t_osc)^(3/2) * F_inf/F0^2 + 1/(1 + (x/t_c)^r), with x = tp * H^beta",
        "results": sorted(compare_rows, key=lambda row: row["vw"]),
        "compare_dir": str(ROOT / "results_collapse_fixed_tc_vw_compare_tosc"),
    }
    cf.save_json(ROOT / "results_collapse_fixed_tc_vw_compare_tosc" / "final_summary.json", summary)
