...
OUTDIR = ROOT / "results_vw_fullshift"
...
def load_fit():
    ...
    if not path.exists():
        raise FileNotFoundError(f"Missing full-shift fit JSON: {path}")
...
def xi_curve(beta_grid, theta, h_value, vw, f0, fit_payload, t_osc):
    ...
    plateau = np.power(max(scale, 1.0e-18), 1.5) * np.power(tp_grid / float(t_osc), 1.5) * finf / max(float(f0) ** 2, 1.0e-18)
    transient = 1.0 / (
...
def plot_grid_for_h(df, theta_values, fit_payload, h_value, t_osc, outpath):
    ...
    fig.suptitle(
        rf"Full-shift fit vs data, $H_*={h_value:.1f}$"
        + "\n"
        + rf"$t_p \to s(v_w)\,t_p$, $r={fit_payload['r']:.3f}$, $t_c={fit_payload['t_c']:.3f}$",
        y=0.98,
    )
...
def plot_separate(df, theta_values, fit_payload, t_osc, outdir):
    ...
    for h_value in np.sort(df["H"].unique()):
        ...
        for theta in np.sort(theta_values):
            ...
            ax.set_xlabel(r"$\beta/H_*$")
            ax.set_ylabel(r"$\xi$")
            ax.set_title(rf"Full-shift: $H_*={h_value:.1f}$, $\theta={theta:.3f}$")
            ax.legend(frameon=False, fontsize=7, ncol=2)
            fig.tight_layout()

            h_tag = f"H{h_value:.1f}".replace(".", "p")
            theta_tag = f"theta_{theta:.10f}".replace(".", "p")
            filepath = outdir / f"xi_vs_betaH_fullshift_{h_tag}_{theta_tag}.png"
...
def main():
    ...
    for h_value in [1.5, 2.0]:
        out = OUTDIR / f"xi_vs_betaH_fullshift_H{str(h_value).replace('.', 'p')}.png"
        plot_grid_for_h(df, theta_values, fit_payload, h_value, args.t_osc, out)
        files.append(str(out))

    separate_dir = OUTDIR / "xi_vs_betaH_fullshift_separate"
    index_df = plot_separate(df, theta_values, fit_payload, args.t_osc, separate_dir)
    index_df.to_csv(OUTDIR / "xi_vs_betaH_fullshift_index.csv", index=False)

    summary = {
        "status": "ok",
        "files": files,
        "separate_dir": str(separate_dir),
        "index_csv": str(OUTDIR / "xi_vs_betaH_fullshift_index.csv"),
        "rel_rmse": fit_payload["rel_rmse"],
        "r": fit_payload["r"],
        "t_c": fit_payload["t_c"],
        "scales": fit_payload["scales"],
        "plateau_factor_by_vw": {
            key: float(val ** 1.5) for key, val in fit_payload["scales"].items()
        },
    }
    save_json(OUTDIR / "xi_vs_betaH_fullshift_plot_summary.json", summary)
...
if __name__ == "__main__":
    try:
        ...
        save_json(OUTDIR / "xi_vs_betaH_fullshift_plot_error.json", payload)
