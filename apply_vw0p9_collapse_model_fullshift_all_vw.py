...
OUTDIR = ROOT / "results_vw0p9_model_fullshift_all_vw"
...
def parse_args():
    p = argparse.ArgumentParser(
        description="Apply the frozen vw=0.9 collapse fit to all vw values while fitting one full time scale s(v_w) per wall speed."
    )
...
def predict_with_scale(sub_df, model, s):
    work = sub_df.copy()
    ...
    work["x"] = x
    yfit, _ = collapse.xi_model_from_params(work, model["theta_values"], model["params"])
    return yfit, x


def fit_scale_for_vw(sub_df, model, s_min, s_max, fixed=None):
    if fixed is not None:
        yfit, _ = predict_with_scale(sub_df, model, fixed)
        return {"s": float(fixed), "success": True, "message": "fixed", "rel_rmse": rel_rmse(sub_df["xi"], yfit)}

    y = sub_df["xi"].to_numpy(dtype=np.float64)

    def objective(s):
        yfit, _ = predict_with_scale(sub_df, model, float(s))
        return rel_rmse(y, yfit)

    res = minimize_scalar(objective, bounds=(float(s_min), float(s_max)), method="bounded", options={"xatol": 1.0e-4})
    s_best = float(res.x) if res.success else 1.0
    return {
        "s": s_best,
        "success": bool(res.success),
        "message": str(getattr(res, "message", "")),
        "rel_rmse": float(objective(s_best)),
    }


def apply_scales(df, model, scales):
    out = df.copy()
    ...
    for vw, sub_idx in out.groupby("v_w", sort=False).groups.items():
        s = float(scales[f"{float(vw):.1f}"]["s"])
        sub = out.loc[sub_idx].copy()
        yfit, x = predict_with_scale(sub, model, s)
        x_vals[np.asarray(sub_idx, dtype=np.int64)] = x
        yfit_vals[np.asarray(sub_idx, dtype=np.int64)] = yfit
    out["x"] = x_vals
    out["xi_fit_fullshift"] = yfit_vals
    return out
...
def plot_collapse_overlay(df, outdir: Path, dpi: int, beta: float):
    ...
    for ax, theta in zip(axes, theta_values):
        ...
        for vw in vw_values:
            for h in h_values:
                ...
                ax.plot(cur["x"], cur["xi_fit_fullshift"], color=colors[float(vw)], lw=1.6, alpha=0.95)
        ax.set_xscale("log")
        ax.set_yscale("log")
        ax.grid(alpha=0.25)
    ...
    fig.suptitle(rf"Frozen $v_w=0.9$ model with fitted full shift $s(v_w)$, $\beta={beta:.4f}$", y=0.995)
    fig.tight_layout(rect=[0, 0, 1, 0.93])
    fig.savefig(outdir / "collapse_overlay_vw0p9model_fullshift_allvw.png", dpi=dpi)
    plt.close(fig)
    ...
    for h_value in h_values:
        ...
        for ax, theta in zip(axes, theta_values):
            ...
            for vw in vw_values:
                ...
                ax.plot(cur["beta_over_H"], cur["xi_fit_fullshift"], color=colors[float(vw)], lw=1.8)
                rows.append({"H": float(h_value), "theta": float(theta), "v_w": float(vw), "rel_rmse": rel_rmse(cur["xi"], cur["xi_fit_fullshift"])})
            ax.set_xscale("log")
            ax.grid(alpha=0.25)
            ax.set_title(rf"$\theta={theta:.3f}$")
        ...
        fig.suptitle(rf"Frozen $v_w=0.9$ model with full shift in raw $\xi(\beta/H_*)$, $H_*={h_value:.1f}$, $\beta={beta:.4f}$", y=0.995)
        fig.tight_layout(rect=[0, 0, 1, 0.94])
        htag = str(float(h_value)).replace(".", "p")
        fig.savefig(outdir / f"xi_vs_betaH_vw0p9model_fullshift_H{htag}.png", dpi=dpi)
        plt.close(fig)
    return rows
...
def main():
    ...
    print("[fit] fitting one full scale s(v_w) per wall speed")
    scales = {}
    for vw, sub in df.groupby("v_w", sort=True):
        fixed = 1.0 if np.isclose(float(vw), 0.9, atol=1.0e-12, rtol=0.0) else None
        scales[f"{float(vw):.1f}"] = fit_scale_for_vw(sub.copy(), model, args.s_min, args.s_max, fixed=fixed)

    print("[apply] building predictions")
    out = apply_scales(df, model, scales)
    ...
    rmse_by_vw = [{"v_w": float(vw), "rel_rmse": rel_rmse(sub["xi"], sub["xi_fit_fullshift"])} for vw, sub in out.groupby("v_w", sort=True)]
    rmse_by_h = [{"H": float(h), "rel_rmse": rel_rmse(sub["xi"], sub["xi_fit_fullshift"])} for h, sub in out.groupby("H", sort=True)]
    rmse_by_theta = [{"theta": float(theta), "rel_rmse": rel_rmse(sub["xi"], sub["xi_fit_fullshift"])} for theta, sub in out.groupby("theta", sort=True)]
    ...
    summary = {
        ...
        "r": float(model["r"]),
        "scales": scales,
        "global_rel_rmse": rel_rmse(out["xi"], out["xi_fit_fullshift"]),
        "rmse_by_vw": rmse_by_vw,
        "rmse_by_H": rmse_by_h,
        "rmse_by_theta": rmse_by_theta,
        "outputs": {
            "collapse_overlay": str(outdir / "collapse_overlay_vw0p9model_fullshift_allvw.png"),
            "raw_H1p0": str(outdir / "xi_vs_betaH_vw0p9model_fullshift_H1p0.png"),
            "raw_H1p5": str(outdir / "xi_vs_betaH_vw0p9model_fullshift_H1p5.png"),
            "raw_H2p0": str(outdir / "xi_vs_betaH_vw0p9model_fullshift_H2p0.png"),
