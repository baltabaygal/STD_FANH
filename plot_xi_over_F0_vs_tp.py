...
OUTDIR = ROOT / "results_vw_xi_over_F0_vs_tp"
VW_TAGS = ["v3", "v5", "v7", "v9"]
H_VALUES = [1.0, 1.5, 2.0]


def parse_args():
    p = argparse.ArgumentParser(description="Plot lattice xi/F0 versus inferred tp, grouped by H* and theta.")
    ...
    for h in h_values:
        ...
        for ax, theta in zip(axes, theta_values):
            ...
            for vw, cur in sub.groupby("v_w", sort=True):
                ...
                beta_vals = cur["beta_over_H"].to_numpy(dtype=np.float64)
                tp_vals = cur["tp"].to_numpy(dtype=np.float64)
                y_vals = cur["xi_over_F0"].to_numpy(dtype=np.float64)
                for tp0, y0, beta0 in zip(tp_vals, y_vals, beta_vals):
                    ax.scatter(
                        [tp0],
                        [y0],
                        color=colors[float(vw)],
                        marker=marker_map.get(float(beta0), "o"),
                        s=28,
                        alpha=0.95,
                    )
                if len(cur) >= 2:
        ...
        fig.legend(vw_handles, vw_labels, loc="upper center", ncol=min(len(vw_labels), 4), frameon=False)
        fig.suptitle(rf"Lattice $\xi/F_0$ vs inferred $t_p$, $H_*={float(h):.1f}$", y=0.995)
        fig.tight_layout(rect=[0, 0, 1, 0.94])
        tag = str(float(h)).replace(".", "p")
        fig.savefig(outdir / f"xi_over_F0_vs_tp_H{tag}.png", dpi=dpi)
        plt.close(fig)
...
def plot_separate(df: pd.DataFrame, outdir: Path, dpi: int):
    ...
    for (h, theta), sub in df.groupby(["H", "theta"], sort=True):
        ...
        path = outdir / f"xi_over_F0_vs_tp_{h_tag}_{theta_tag}.png"
...
def main():
    ...
    print("[plot] writing xi/F0 vs inferred tp diagnostics")
    summary_rows = plot_main(df, outdir, args.dpi)
    separate_rows = plot_separate(df, outdir / "xi_over_F0_vs_tp_separate", args.dpi)
    pd.DataFrame(separate_rows).to_csv(outdir / "xi_over_F0_vs_tp_index.csv", index=False)
    ...
    summary = {
        ...
        "outputs": {
            **{f"H{str(float(h)).replace('.', 'p')}": str(outdir / f"xi_over_F0_vs_tp_H{str(float(h)).replace('.', 'p')}.png") for h in np.sort(df["H"].unique())},
            "separate_dir": str(outdir / "xi_over_F0_vs_tp_separate"),
            "index_csv": str(outdir / "xi_over_F0_vs_tp_index.csv"),
        },
