...
OUTDIR = ROOT / "results_vw_xi_over_F0"
VW_TAGS = ["v3", "v5", "v7", "v9"]
H_VALUES = [1.0, 1.5, 2.0]


def parse_args():
    p = argparse.ArgumentParser(description="Plot lattice xi/F0 versus v_w, grouped by H* and theta.")
...
def plot_main(df: pd.DataFrame, outdir: Path, dpi: int):
    ...
    for h in h_values:
                ...
                summary_rows.append(
                    {
                        ...
                        "ratio_max_over_min": float(
                            np.max(cur["xi_over_F0"].to_numpy(dtype=np.float64))
                            / np.maximum(np.min(cur["xi_over_F0"].to_numpy(dtype=np.float64)), 1.0e-18)
                        ),
                    }
                )
            ax.grid(alpha=0.25)
        ...
        fig.suptitle(rf"Lattice $\xi/F_0$ vs $v_w$, $H_*={h:.1f}$", y=0.995)
        fig.tight_layout(rect=[0, 0, 1, 0.95])
        tag = str(float(h)).replace(".", "p")
        fig.savefig(outdir / f"xi_over_F0_vs_vw_H{tag}.png", dpi=dpi)
        plt.close(fig)
    return summary_rows
...
def plot_separate(df: pd.DataFrame, outdir: Path, dpi: int):
    ...
    for (h, theta), sub in df.groupby(["H", "theta"], sort=True):
        ...
        path = outdir / f"xi_over_F0_vs_vw_{h_tag}_{theta_tag}.png"
...
def main():
    ...
    print("[plot] writing xi/F0 vs vw diagnostics")
    summary_rows = plot_main(df, outdir, args.dpi)
    separate_rows = plot_separate(df, outdir / "xi_over_F0_vs_vw_separate", args.dpi)
    pd.DataFrame(separate_rows).to_csv(outdir / "xi_over_F0_vs_vw_index.csv", index=False)
    ...
    summary = {
        ...
        "outputs": {
            **{f"H{str(float(h)).replace('.', 'p')}": str(outdir / f"xi_over_F0_vs_vw_H{str(float(h)).replace('.', 'p')}.png") for h in np.sort(df["H"].unique())},
            "separate_dir": str(outdir / "xi_over_F0_vs_vw_separate"),
            "index_csv": str(outdir / "xi_over_F0_vs_vw_index.csv"),
        },
