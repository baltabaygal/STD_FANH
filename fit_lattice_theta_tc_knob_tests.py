...
OUTDIR_DEFAULT = ROOT / "knob_tc"
REFERENCE_DIR_DEFAULT = ROOT / "results_lattice_theta_tc_rvw_tests_beta0_tcmax300"
VW_TAGS = ["v3", "v5", "v7", "v9"]
...
def parse_args():
    p = argparse.ArgumentParser(
        description="Test a compact timing knob tc(theta0,vw)=tc0(vw) h(theta0)^gamma against existing r(vw) lattice fits."
    )
...
def tc_factorized_init(tc_grid: np.ndarray, theta_values: np.ndarray):
    tc_grid = np.asarray(tc_grid, dtype=np.float64)
    n_vw, n_theta = tc_grid.shape
    z = np.log(np.maximum(h_theta(theta_values), 1.0e-18))
    design = np.zeros((n_vw * n_theta, n_vw + 1), dtype=np.float64)
    y = np.log(np.maximum(tc_grid.reshape(-1), 1.0e-18))
    row = 0
    for i in range(n_vw):
    ...
    tc0_by_vw = np.exp(coeff[:n_vw])
    gamma = float(coeff[-1])
    return np.asarray(tc0_by_vw, dtype=np.float64), gamma
...
def unpack_params(params: np.ndarray, n_vw: int, n_theta: int):
    ...
    return r_by_vw, tc0_by_vw, gamma, f_infty_theta


def build_param_vector(init_r_by_vw: np.ndarray, init_tc0_by_vw: np.ndarray, init_gamma: float, init_f_infty: np.ndarray):
    ...
    x0 = np.concatenate([r0, tc0, np.asarray([float(init_gamma)], dtype=np.float64), finf0])
...
def model_details(meta, fixed_beta: float, params: np.ndarray, kernel: str):
    ...
    r_by_vw, tc0_by_vw, gamma, f_infty_theta = unpack_params(params, n_vw, n_theta)
    x, xi_scale = shared_rvw.x_and_xi_scale(meta, fixed_beta)
    r_eff = r_by_vw[meta["vw_idx"]]
    tc_eff = tc0_by_vw[meta["vw_idx"]] * np.power(np.maximum(meta["h_theta_row"], 1.0e-18), float(gamma))
    ...
    tc_by_vw_theta = tc0_by_vw[:, None] * np.power(np.maximum(meta["h_theta_values"][None, :], 1.0e-18), float(gamma))
    return x, xi_scale, tc_eff, r_eff, f_tilde, xi_fit, r_by_vw, tc0_by_vw, gamma, f_infty_theta, tc_by_vw_theta


def fit_case(meta, fixed_beta: float, init_r_by_vw: np.ndarray, init_tc0_by_vw: np.ndarray, init_gamma: float, init_f_infty: np.ndarray, tc_max: float, kernel: str):
    x0, lower, upper = build_param_vector(init_r_by_vw, init_tc0_by_vw, init_gamma, init_f_infty)
    n_vw = len(meta["vw_values"])
    ...
    x, xi_scale, tc_eff, r_eff, f_tilde, xi_fit, r_by_vw, tc0_by_vw, gamma, f_infty_theta, tc_by_vw_theta = model_details(
    ...
    payload = {
        ...
        "f_tilde_form": "f_tilde = f_infty(theta0) + F0(theta0) / ((2 x / (3 t_osc))^(3/2) * D), with tc(theta0,v_w) = tc0(v_w) * h(theta0)^gamma and r = r(v_w)",
...
def build_prediction_frame(df, meta, fit_payload: dict):
    ...
    n_vw = len(meta["vw_values"])
    n_theta = len(meta["theta_values"])
    _, _, _, f_infty_theta = unpack_params(fit_payload["result_x"], n_vw, n_theta)
...
def plot_tc0_by_vw(case_summary: dict, outpath: Path, dpi: int):
    fig, ax = plt.subplots(figsize=(6.0, 4.0))
    vw_values = [0.3, 0.5, 0.7, 0.9]
    vals = [case_summary["tc0_by_vw"][f"{vw:.1f}"] for vw in vw_values]
    ax.plot(vw_values, vals, "o-", lw=1.8, color="tab:blue")
    ax.set_xlabel(r"$v_w$")
    ax.set_ylabel(r"$t_c^{(0)}(v_w)$")
    ax.set_title(rf"$\gamma={case_summary['gamma']:.3f}$")
    ax.grid(alpha=0.25)
    fig.savefig(outpath, dpi=dpi)
    plt.close(fig)
...
def make_comparison_sheet(case_dirs: dict[str, Path], outpath: Path, dpi: int):
    ...
    for ax, (title, path) in zip(axes.ravel(), items):
        ...
        ax.axis("off")
    fig.suptitle(r"$t_c$ knob comparison", fontsize=16)
    fig.tight_layout(rect=[0, 0, 1, 0.98])
    fig.savefig(outpath, dpi=dpi)
    plt.close(fig)


def copy_reference_case(reference_dir: Path, outdir: Path, mode: str):
    src = reference_dir / mode
    dst = outdir / mode
    if not src.exists():
        raise FileNotFoundError(f"Missing reference case directory: {src}")
...
def main():
    ...
    try:
        ...
        init_tc0_by_vw, init_gamma = tc_factorized_init(init_tc_grid, theta_values)
        init_f_infty = f_infty_from_case(ref_theta, theta_values)

        case_dirs = {}
        case_summaries = {
            "rvw_globaltc": reference_summary["cases"]["rvw_globaltc"],
            "rvw_theta": reference_summary["cases"]["rvw_theta"],
        }
        case_dirs["rvw_globaltc"] = copy_reference_case(reference_dir, outdir, "rvw_globaltc")
        case_dirs["rvw_theta"] = copy_reference_case(reference_dir, outdir, "rvw_theta")
        ...
        knob_dir = outdir / "rvw_theta_hgamma"
        knob_dir.mkdir(parents=True, exist_ok=True)
        pred = build_prediction_frame(df, meta, knob)
        pred.to_csv(knob_dir / "predictions.csv", index=False)
        shared_rvw.plot_collapse_overlay(
            pred,
            theta_values,
            vw_values,
            knob,
            knob_dir / "collapse_overlay.png",
            args.dpi,
            args.fixed_beta,
            rf"$r(v_w)$, $t_c^{{(0)}}(v_w)\,h(\theta_0)^{{\gamma}}$, kernel {shared_rvw.kernel_label(args.transient_kernel)}",
        )
        shared_rvw.plot_raw(pred, theta_values, vw_values, knob_dir, "xi_vs_betaH", args.dpi)
        raw_rows, mean_by_h, mean_by_h_vw = rmse_tables(pred)
        knob_summary = summarize_payload(knob, theta_values, vw_values, ode)
        knob_summary["label"] = r"$r(v_w)$, $t_c^{(0)}(v_w)\,h(\theta_0)^\gamma$"
        knob_summary["mean_raw_rmse_by_h"] = mean_by_h
        knob_summary["mean_raw_rmse_by_h_vw"] = mean_by_h_vw
        knob_summary["raw_plot_rmse"] = raw_rows
        case_summaries["rvw_theta_hgamma"] = knob_summary
        case_dirs["rvw_theta_hgamma"] = knob_dir
        save_json(knob_dir / "final_summary.json", knob_summary)
        ...
        plot_tc0_by_vw(knob_summary, outdir / "tc0_by_vw.png", args.dpi)
