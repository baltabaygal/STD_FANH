...
OUTDIR = ROOT / "results_vw_overlay_quadwarped"
VW_TAGS = ["v3", "v5", "v7", "v9"]
...
def parse_args():
    p = argparse.ArgumentParser(description="Overlay lattice xi after fitting per-vw quadratic log-time warps.")
...
def monotonic_margin(tp_all, b, c):
    ltp = np.log(np.maximum(np.asarray(tp_all, dtype=np.float64), 1.0e-18))
    deriv = float(b) + 2.0 * float(c) * ltp
    return float(np.min(deriv))


def quadwarp_residuals(log_s, b, c, pairs, beta):
    residuals = []
    beta = float(beta)
    for pair in pairs:
        h = float(pair["H"])
        x_ref = pair["ref_tp"] * (h ** beta)
        x_cur = warp_tp(pair["tp"], log_s, b, c) * (h ** beta)
...
def fit_quadwarp_for_pairs(pairs, beta):
    if not pairs:
        return {
            "log_s": np.nan,
            "s": np.nan,
    ...
    tp_all = np.concatenate([pair["tp"] for pair in pairs]).astype(np.float64)

    def objective(x):
        log_s, b, c = [float(v) for v in x]
        margin = monotonic_margin(tp_all, b, c)
        if not np.isfinite(margin) or margin <= 0.05:
            return 1.0e9 + 1.0e6 * (0.05 - margin if np.isfinite(margin) else 1.0)
        resid = quadwarp_residuals(log_s, b, c, pairs, beta)
        if resid.size == 0:
            return 1.0e9
    ...
    res = minimize(
        objective,
        x0=np.array([0.0, 1.0, 0.0], dtype=np.float64),
        method="L-BFGS-B",
        bounds=[(-2.0, 2.0), (0.4, 1.6), (-0.25, 0.25)],
    )
    log_s, b, c = [float(v) for v in res.x]
    resid = quadwarp_residuals(log_s, b, c, pairs, beta)
    return {
        "log_s": log_s,
        ...
        "monotonic_margin": monotonic_margin(tp_all, b, c),
    }


def fit_quadwarps(df, ref_vw, beta):
    pair_map = smear.build_alignment_pairs(df, ref_vw)
    params = {f"{ref_vw:.1f}": {"log_s": 0.0, "s": 1.0, "b": 1.0, "c": 0.0}}
    rows = [
        {
            "v_w": float(ref_vw),
            "s": 1.0,
    ...
    for vw, pairs in pair_map.items():
        fit = fit_quadwarp_for_pairs(pairs, beta)
        params[f"{float(vw):.1f}"] = {"log_s": fit["log_s"], "s": fit["s"], "b": fit["b"], "c": fit["c"]}
...
def plot_overlay(df, params, beta, outpath: Path, title_tag: str):
    ...
    for ax, theta in zip(axes, theta_subset):
        ...
        for vw in vw_values:
            ...
            for h in h_values:
                ...
                if cur.empty:
                    continue
                x = warp_tp(cur["tp"].to_numpy(dtype=np.float64), rec["log_s"], rec["b"], rec["c"]) * np.power(
        ...
        if abs(float(beta)) < 1.0e-12:
            ax.set_xlabel(r"$x = \exp(a+b\ln t_p + c \ln^2 t_p)$")
        else:
            ax.set_xlabel(r"$x = \exp(a+b\ln t_p + c \ln^2 t_p)\, H_*^\beta$")
        ax.set_ylabel(r"$\xi$")
    ...
    if abs(float(beta)) < 1.0e-12:
        fig.suptitle(rf"Lattice $\xi$ after fitted per-$v_w$ quadratic log-time warp{suffix}", y=0.995)
    else:
        fig.suptitle(rf"Lattice $\xi$ after fitted per-$v_w$ quadratic log-time warp and $H^\beta$ rescaling, $\beta={beta:.4f}{suffix}$", y=0.995)
...
def main():
    ...
    params, rows = fit_quadwarps(df, ref_vw, args.beta)

    outpath = outdir / "lattice_xi_vs_x_all_vw_quadwarped.png"
    ...
    (outdir / "lattice_quadwarp_summary.json").write_text(json.dumps(summary, indent=2, sort_keys=True))
...
if __name__ == "__main__":
    try:
        ...
        outdir.mkdir(parents=True, exist_ok=True)
        (outdir / "lattice_quadwarp_error.json").write_text(json.dumps(payload, indent=2, sort_keys=True))
