...
DEFAULT_OUTDIR = ROOT / "results_ode_direct_transition_tc_norm"
DEFAULT_TABLES = [
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description="Fit the direct ODE tp-scan data with the tc-normalized transition ansatz."
    )
...
def estimate_finf_init(df: pd.DataFrame, theta_values: np.ndarray, tc0: float) -> np.ndarray:
    ...
    for i, theta in enumerate(theta_values):
        ...
        out[i] = max(float(np.median(tail["xi"].to_numpy(dtype=np.float64) * tail["F0"].to_numpy(dtype=np.float64) / x)), 1.0e-8)
    return out
...
def fanh_model(tp: np.ndarray, f0: np.ndarray, finf: np.ndarray, tc: float, r: float) -> np.ndarray:
    x = np.maximum(tp / max(tc, 1.0e-18), 1.0e-18)
    return finf + f0 / (np.power(x, 1.5) * (1.0 + np.power(x, r)))
...
def xi_model(params: np.ndarray, df: pd.DataFrame, theta_values: np.ndarray) -> np.ndarray:
    ...
    x = np.maximum(tp / max(tc, 1.0e-18), 1.0e-18)
    return np.power(x, 1.5) * finf[theta_idx] / np.maximum(f0, 1.0e-18) + 1.0 / (1.0 + np.power(x, r))
...
def fit_global(df: pd.DataFrame, theta_values: np.ndarray, tc0: float, r0: float) -> dict:
    ...
    upper = np.concatenate([[100.0, 50.0], np.full(len(theta_values), 1.0e3)])
...
def bootstrap_fit(df: pd.DataFrame, theta_values: np.ndarray, fit_bundle: dict, nboot: int, n_jobs: int) -> dict:
    ...
    seeds = np.arange(nboot, dtype=np.int64) + 13579
...
def plot_xi_by_H(df: pd.DataFrame, payload: dict, outdir: Path) -> None:
    for h in sorted(df["H"].unique()):
        ...
        for ax, theta in zip(axes.flat, theta_values):
            ...
            xi_fit = np.power(x, 1.5) * finf / max(f0, 1.0e-18) + 1.0 / (1.0 + np.power(x, payload["r"]))
            ax.plot(tp, xi, "o", ms=3.2, color="tab:blue", label="ODE data")
            ax.plot(tp, xi_fit, lw=2.0, color="black", label="tc-norm fit")
            ax.set_xscale("log")
            ax.grid(alpha=0.25)
        ...
        fig.suptitle(
            rf"Direct ODE tc-normalized fit, $H_*={h:.1f}$" + "\n" + rf"$t_c={payload['t_c']:.3f},\ r={payload['r']:.3f}$",
            y=0.98,
        )
...
def plot_fanh_by_H(df: pd.DataFrame, payload: dict, outdir: Path) -> None:
    for h in sorted(df["H"].unique()):
        ...
        for ax, theta in zip(axes.flat, theta_values):
            ...
            x = np.maximum(tp / payload["t_c"], 1.0e-18)
            # The direct ODE table stores f_anh_PT = Ea3_PT / [V(theta) tp^(3/2)],
            # while F0 = f_anh_noPT is normalized at t_osc. Convert xi_model back
            # to that stored fanh convention before plotting.
            fanh_fit = np.power(t_osc / payload["t_c"], 1.5) * finf + f0 * np.power(
                np.maximum(t_osc / tp, 1.0e-18), 1.5
            ) / (1.0 + np.power(x, payload["r"]))
            ax.plot(tp, fanh, "o", ms=3.2, color="tab:green", label="ODE data")
            ax.plot(tp, fanh_fit, lw=2.0, color="black", label="tc-norm fit")
            ax.set_xscale("log")
            ax.set_yscale("log")
            ax.grid(alpha=0.25)
            ax.set_title(rf"$\theta={theta:.3f}$")
        ...
        axes.flat[0].legend(frameon=False, fontsize=8)
        fig.suptitle(
            rf"Direct ODE tc-normalized fanh fit, $H_*={h:.1f}$" + "\n" + rf"$t_c={payload['t_c']:.3f},\ r={payload['r']:.3f}$",
            y=0.98,
        )
        fig.tight_layout(rect=[0.0, 0.0, 1.0, 0.93])
        tag = f"H{h:.1f}".replace(".", "p")
...
def main() -> None:
    ...
    try:
        ...
        fit_bundle = fit_global(df, theta_values, args.tc0, args.r0)
        ...
        summary = {
            ...
            "fit": payload,
            "bootstrap": boot,
            "note": "Direct ODE fit of xi(tp) = (tp/t_c)^(3/2) F_inf/F0 + 1/(1 + (tp/t_c)^r). The ODE table stores f_anh_PT = Ea3_PT/[V(theta) tp^(3/2)], so the plotted fanh conversion is fanh_model = F0 * xi_model / (tp/t_osc)^(3/2) = (t_osc/t_c)^(3/2) F_inf + F0 (t_osc/tp)^(3/2) / (1 + (tp/t_c)^r).",
        }
